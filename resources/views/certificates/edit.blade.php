...
<div class="container mx-auto p-6">
    ...
    <form action="{{ route('certificates.update', $certificate->id) }}" method="POST" class="bg-white shadow-md rounded p-6">
        @csrf
        @method('PUT')
        <div class="mb-4">
            ...
            <select id="enrollment_id" name="enrollment_id" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                ...
                @foreach ($enrollments as $enrollment)
                    <option value="{{ $enrollment->id }}" {{ $certificate->enrollment_id == $enrollment->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="certificate_code" name="certificate_code" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ $certificate->certificate_code }}" required>
        </div>
        <div>
