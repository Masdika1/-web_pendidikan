            ...
            <tbody class="divide-y divide-gray-200 bg-gray-50">
                ...
                <tr class="hover:bg-gray-100 transition-all duration-300">
                    ...
                    <td class="px-6 py-4 text-sm flex justify-center space-x-4">
                        <a href="{{ route('userstudent.show', $user->id) }}" class="px-4 py-2 bg-green-500 text-white rounded-lg shadow-lg hover:shadow-xl hover:bg-green-600 transform hover:scale-105 transition-all">Detail</a>
                        <a href="{{ route('userstudent.edit', $user->id) }}" class="px-4 py-2 bg-yellow-500 text-white rounded-lg shadow-lg hover:shadow-xl hover:bg-yellow-600 transform hover:scale-105 transition-all">Edit</a>
                        <form action="{{ route('userstudent.destroy', $user->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
